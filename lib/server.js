...
class VHCServer extends EventEmitter{
  ...
  setupExpress() {
    ...
    for(let path in self.pathPrefixFileMap){
      ...
      self.express.use(path,function(req,res,next){
        ...
        self.sendFileToRes(req,res,filePath)
      })
    }
  ...
  sendFileToRes(req,res,filePath) {
    const self = this
    fsp.readFile(filePath)
      .then(data => {
        const body = data.toString()
        return self.fillIncludeUrls(req,body)
      }).then(body => {
        res.send(body)
      })
      .catch(err => {
  ...
  fillIncludeUrls(req,body) {
    const self = this
    const tagRegExp = /\<vhc-include-url\>(.*)\<\/vhc-include-url\>/gi
    const vhcTagMatches = body.match(tagRegExp)||[]
    if(vhcTagMatches.length == 0){
      return body
    }
    const tag = vhcTagMatches[0]
    const requestUrl = req.headers['x-forwarded-proto'] || req.protocol + '://' + req.get('host') + req.originalUrl;
    const url = tag.replace(tagRegExp,"$1") + '?url=' + encodeURIComponent(requestUrl)
    console.log(url)
        ...
        if(res.status == 200){
          return res.text()
            .then(includeBody => {
              ...
              const newBody = body.split(tag).join(includeBody)
              return self.fillIncludeUrls(req,newBody)
            })
        }
        return body.split(tag).join('')
      })
      .catch(err => {
        console.log(err)
        return body.split(tag).join('')
      ...
      entries.forEach(function(entry){
        ...
        if(!entry.isDirectory()){
          promises.push(mkdirpPromise(fullDir).then(function(){
            ...
            // console.log(fullPath)
            const newBuffer = blob.content()
            return fsp.readFile(fullPath)
              .then(function(currentBuffer){
                const equal = newBuffer.equals(currentBuffer)
                if(!equal){
                  return fsp.writeFile(fullPath,newBuffer)
                }
                return Promise.resolve()
              })
              .catch(err => {
