...
class VHCServer extends EventEmitter{
      ...
      .then(function(versions){
        ...
        fs.watch(self.sourceGitPath,{
          ...
          if(!self.renderedFiles[fullPath]){
            ...
            const hasRenderedTemplate = self.templateFiles['current']
              && self.templateFiles['current'].indexOf(fullPath) != -1
            if(!hasRenderedTemplate){
              self.templateFiles['current'] = []
            }
  ...
  emitReadyOrError() {
    if(this.availableVersions[this.currentVersion]){
      this.ready = true
      this.emit('ready')
    }
    else{
